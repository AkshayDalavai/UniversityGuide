...
public class CommentsService {
	...
	@Autowired
	public CommentsService(CommentsRepository theCommentsRepository, UserRepository theUserRepository, PostsRepository thePostRepository)
	{
		commentsRepository = theCommentsRepository;
		userRepository = theUserRepository;
		postsRepository = thePostRepository;
	}
	...
	public CommentsDto createComments(CommentsDto commentsDto)
	{
		...
		Posts thePosts = null;
		Optional<Posts> resultPost = postsRepository.findById(postsId);
		if(resultPost.isPresent()) {
			thePosts = resultPost.get();
		}
		else {
			throw new RuntimeException("Did not find post id - " + postsId);
		}
		...
		commentsDto.setLikesCount(0);
		
		commentsDto.getPosts().setHasComments(true);
		int a = commentsDto.getPosts().getCommentsCount();
		commentsDto.getPosts().setCommentsCount(a+1);
		String createdBy = commentsDto.getPosts().getCreatedBy();
		commentsDto.getPosts().setCreatedBy(createdBy);
		postsRepository.save(FromDtoConverter.fromPostsDtoConverter(commentsDto.getPosts()));
		
		Comments theComments = commentsRepository.save(FromDtoConverter.fromCommentsDtoConverter(commentsDto));
		
		commentsDto.setCreatedDate(theComments.getCreatedDate());
		commentsDto.setLastModifiedDate(theComments.getLastModifiedDate());
		commentsDto.setCreatedBy(theComments.getCreatedBy());
		 
		return ToDtoConverter.commentsToDtoConverter(theComments);
	}
